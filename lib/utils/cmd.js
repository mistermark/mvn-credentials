...
function executeCmd(command, args, options) {
    ...
    if(options && options.detached === true && !options.last) {
        outputPath = path.join(process.cwd(), options.prefixLog + '-out.log');
        errorPath = path.join(process.cwd(), options.prefixLog + '-err.log');
        outLog = fs.openSync( outputPath, 'a');
        errLog = fs.openSync( errorPath, 'a');
        options.stdio = [ 'ignore', outLog, errLog ];
    }
    // Buffer output, reporting progress
    _proc = cp.spawn(command, args, options);
    if(options && options.detached){
        global.killOnExit.processes.push(_proc);
    }
    if(options && options.detached === true && !options.last){
        readOutputStream = fs.createReadStream(outputPath);
        readOutputStream.on('data', function (data) {
            data = data.toString();
            deferred.notify(data);
            stdout += data;
        });
        readErrorStream = fs.createReadStream(errorPath);
